...
import { FileSearch2, LayoutDashboard, Timer } from "lucide-react";
    ...
    <div className="bg-[#f4f9fa] py-16">
      ...
      <Container className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {features.map(({ title, description, Icon }) => (
          <div key={title} className="px-8 py-10 rounded-lg bg-white space-y-4">
            <div className="p-4 rounded-md bg-[#e2f3f2] w-fit">
              <Icon className="w-8 h-8 text-[#00aeab]" />
            </div>
            <h3 className="text-2xl font-semibold">{title}</h3>
            <p className="text-lg font-medium text-[#1f6064]">{description}</p>
          </div>
        ))}
      </Container>
    </div>
